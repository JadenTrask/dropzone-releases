...
export class ForestMap{
  constructor(canvas,{getLocations,getColor,getFound,onSelect,onStatus}){
    Object.assign(this,{canvas,getLocations,getColor,getFound,onSelect,onStatus});this.ctx=canvas.getContext('2d');this.images=new Map();this.frame=0;this.events=new AbortController();const signal=this.events.signal;
    ...
    canvas.addEventListener('pointerdown',e=>{if(e.button!==0)return;canvas.focus();canvas.setPointerCapture(e.pointerId);this.drag={start:this.pointer(e),camera:{...this.camera},moved:false};},{signal});
  ...
  hit(point){return this.getLocations().map(p=>({p,s:this.screen(p)})).filter(({s})=>Math.hypot(s.x-point.x,s.y-point.y)<12).sort((a,b)=>Math.hypot(a.s.x-point.x,a.s.y-point.y)-Math.hypot(b.s.x-point.x,b.s.y-point.y))[0]?.p;}
  ...
  render(){if(!this.camera)return;const ctx=this.ctx;ctx.setTransform(this.dpr,0,0,this.dpr,0,0);ctx.fillStyle='#101c20';ctx.fillRect(0,0,this.width,this.height);
    ...
    for(const p of this.getLocations()){const s=this.screen(p);if(s.x<-12||s.y<-12||s.x>this.width+12||s.y>this.height+12)continue;this.visible++;ctx.globalAlpha=done.has(p.id)?.4:1;ctx.beginPath();ctx.arc(s.x,s.y,p.id===this.selected?9:6,0,Math.PI*2);ctx.fillStyle=this.getColor(p);ctx.fill();ctx.lineWidth=p.id===this.selected?3:1.5;ctx.strokeStyle=p.id===this.selected?'#fff':'#0a1115';ctx.stroke();if(done.has(p.id)){ctx.fillStyle='#fff';ctx.font='10px sans-serif';ctx.textAlign='center';ctx.fillText('✓',s.x,s.y+3);}}
