...
function setTheme(g){document.documentElement.dataset.game=g?.theme||'home';document.documentElement.style.setProperty('--accent',g?accent(g):'#c0e69b');}
...
function card(g){return `<button class="game-card theme-${e(g.theme)}" data-route="${e(g.id)}" style="--card-accent:${accent(g)}"><img src="${e(g.cover)}" alt="" loading="lazy"><span class="game-card-shade"></span><span class="game-card-top"><span>${e(g.eyebrow)}</span><span class="game-status">${g.status==='active'?'AVAILABLE':e(releaseStatus(g).toUpperCase())}</span></span><span class="game-card-copy"><strong>${e(g.name)}</strong><span>${e(g.description)}</span><span class="game-card-cta">${g.status==='active'?(g.kind==='collection'?e(g.childPrompt||('Choose '+g.name)):'Open game'):g.status==='under-construction'?'View':releaseDate(g)} ${icon(g.status==='active'?'arrow':'clock')}</span></span></button>`;}
...
function renderHome(){
  $('#hub-app').innerHTML=`<main class="hub-main"><div class="library-intro"><div><span class="hub-eyebrow">DROPZONE</span><h1>Choose a game.</h1><p>Loadouts, tactical maps, and competitive matches.</p></div><div class="library-emblem" aria-hidden="true"><img src="assets/brand.svg" alt=""></div></div><div class="game-grid root-games">${state.games.filter(g=>!g.parent).map(card).join('')}</div><div class="library-bottom"><div>${icon('shield')}<span><strong>Saved loadouts</strong>Keep builds on this device.</span></div><div>${icon('refresh')}<span><strong>Automatic source checks</strong>Checks on launch and every 15 minutes.</span></div><div>${icon('grid')}<span><strong>More games later</strong>Built to support new games and tools.</span></div></div>${footer()}</main>`;
}
function renderCollection(g){
  const children=state.games.filter(x=>x.parent===g.id);
  $('#hub-app').innerHTML=`<main class="hub-main"><div class="library-intro"><div><span class="hub-eyebrow">${e(g.name.toUpperCase())}</span><h1>${g.id==='cod'?'Call of Duty':'Choose your game.'}</h1><p>Choose a game to view loadouts and official videos.</p></div><span class="collection-mark" aria-hidden="true">${e(g.shortName||g.name)}</span></div><div class="game-grid collection-games">${children.map(card).join('')}</div><div class="collection-note">${icon('target')}<p>${g.id==='cod'?'BO7 public and ranked builds use separate CODMunity feeds. Warzone has its own arsenal and attachment sets.':e(g.description)}</p></div>${footer()}</main>`;
}
...
async function route(id,watchGame=null){
