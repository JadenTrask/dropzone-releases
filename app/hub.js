...
async function route(id,watchGame=null,initialView=null){
  ...
  if(['progression','damage'].includes(id)){const target=state.games.find(g=>g.id===(watchGame||state.watchGame));if(!target?.pages?.includes(id))return;state.watchGame=target.id;}
  ...
  const g=['watch','patches','progression','damage'].includes(id)?state.games.find(g=>g.id===state.watchGame):game();setTheme(g);gameTabs(g,id==='watch'?'videos':['patches','progression','damage'].includes(id)?id:'loadouts');$('#league-app').hidden=id!=='lol';$('#hub-app').hidden=id==='lol';
...
document.addEventListener('click',async event=>{
  ...
  try{
    if(b.dataset.gamePage){const id=$('#game-tabs').dataset.game;return await route(b.dataset.gamePage==='videos'?'watch':['patches','progression','damage'].includes(b.dataset.gamePage)?b.dataset.gamePage:id,id);}
