import {initCommandCenter,mountCommandCenter,leaveCommandCenter,preferredGames} from './command-center.js';
...
import {mountSquad} from './squad-ui.js';
import {mountWardogsPlanner} from './wardogs-planner-ui.js';
...
async function route(id,watchGame=null,initialView=null){
  if(id==='command'&&!watchGame)watchGame=game()?.id;
  const navigation=++navigationRevision;
  leaveCommandCenter();leaveIntel();leaveFinals();leaveWatch();leaveSources();leavePatches();leaveWardogs();leaveWardogsProgression();leaveSiege();leaveForest();
  if(['progression','damage','planner','squad'].includes(id)){const target=state.games.find(g=>g.id===(watchGame||state.watchGame));if(!target?.pages?.includes(id))return;state.watchGame=target.id;}
  ...
  rememberWorkspace(id,['command','progression','damage','planner','squad','watch','patches'].includes(id)?(id==='command'?watchGame:state.watchGame):null);
  ...
  const g=['watch','patches','progression','damage','planner','squad'].includes(id)?state.games.find(g=>g.id===state.watchGame):game();setTheme(g);gameTabs(g,id==='watch'?'videos':['patches','progression','damage','planner','squad'].includes(id)?id:'loadouts');$('#league-app').hidden=id!=='lol';$('#hub-app').hidden=id==='lol';
  location(g,id==='intel'?'My patch alerts':id==='saved'?'Saved loadouts':id==='watch'?g.name+' / Videos':id==='patches'?g.name+' / Patch notes':id==='updates'?'App updates':id==='sources'?'Sources':null);
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(b.dataset.gamePage){const id=$('#game-tabs').dataset.game;return await route(b.dataset.gamePage==='videos'?'watch':['patches','progression','damage','planner','squad'].includes(b.dataset.gamePage)?b.dataset.gamePage:id,id);}
