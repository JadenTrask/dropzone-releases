...
async function route(id,watchGame=null,initialView=null){
  ...
  const g=['watch','patches','progression','server-status','market','damage','planner','squad'].includes(id)?state.games.find(g=>g.id===state.watchGame):game();setTheme(g);gameTabs(g,id==='watch'?'videos':['patches','progression','server-status','market','damage','planner','squad'].includes(id)?id:'loadouts');$('#league-app').hidden=id!=='lol';$('#hub-app').hidden=id==='lol';
  location(g,id==='intel'?'My patch alerts':id==='saved'?'Saved loadouts':id==='watch'?g.name+' / Videos':id==='patches'?g.name+' / News':id==='updates'?'App updates':id==='sources'?'Sources':null);
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(b.dataset.gamePage){const id=$('#game-tabs').dataset.game;return await route(b.dataset.gamePage==='videos'?'watch':['patches','progression','server-status','market','damage','planner','squad'].includes(b.dataset.gamePage)?b.dataset.gamePage:id,id);}
