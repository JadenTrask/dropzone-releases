...
import {applyVisualUpdates} from './visual-updates.js';
...
import weaponArtReplacements from './data/cod/weapon-art-replacements.json' with {type:'json'};
...
function featureStyle(module){
 const file={'siege-ui':'siege.css','sotf-ui':'sotf.css','rocket-league-ui':'rocket-league.css'}[module];if(!file)return Promise.resolve();
...
function setTheme(g){document.documentElement.dataset.game=g?.theme||'home';document.documentElement.style.setProperty('--accent','#ee6038');document.documentElement.style.setProperty('--game-accent',g?accent(g):'#aac9bb');document.querySelectorAll('.app-rail [data-route]').forEach(b=>b.setAttribute('aria-current',b.dataset.route===(g?.parent||g?.id||state.route)?'page':'false'));}
function gameTabs(g,page='loadouts'){showPlayerCount(g?.id==='wardogs');const shell=$('#game-tabs');shell.hidden=!g||g.kind==='collection'||(g.status==='under-construction'||g.id==='gta6');shell.dataset.game=g?.id||'';document.documentElement.dataset.workspace=g?.status==='active'&&['calculator','tacmap','forest'].includes(g?.kind)&&page==='loadouts'?'map':'standard';$('#league-tools').hidden=g?.id!=='lol'||page!=='loadouts';shell.querySelectorAll('[data-game-page]').forEach(b=>{b.classList.toggle('active',b.dataset.gamePage===page);b.setAttribute('aria-current',b.dataset.gamePage===page?'page':'false');if(b.dataset.gamePage==='operators')b.hidden=g?.id!=='siege';if(b.dataset.gamePage==='session')b.hidden=g?.id==='sotf'||g?.status!=='active';if(['operators','progression','server-status','market','damage','planner','squad'].includes(b.dataset.gamePage))b.hidden=!g?.pages?.includes(b.dataset.gamePage);if(b.dataset.gamePage==='patches')b.hidden=g?.patches===false;if(b.dataset.gamePage==='videos')b.hidden=g?.videos===false;if(b.dataset.gamePage==='loadouts')b.textContent=g?.kind==='rocket-league'?'Live tracker':g?.kind==='calculator'?'Calculator':g?.kind==='forest'?'Island map':g?.kind==='tacmap'?'Tactical map':g?.kind==='siege'?'Ranked':g?.kind==='arena'?'Roster':'Loadouts';});$('#game-tabs-name').textContent=g?.shortName||g?.name||'';}
function location(g,label){document.title='Dropzone · '+(label||g?.name||'Library');}
function discoveryArtwork(g){return g.id==='sotf'?`<span class="game-card-map" aria-hidden="true">${Array.from({length:16},(_,i)=>`<span><img src="assets/sotf/map/2-${i%4}-${Math.floor(i/4)}.webp" alt="" loading="lazy"></span>`).join('')}</span>`:`<img src="${e(g.cover)}" alt="" loading="lazy">`;}
function card(g){const tools={cod:'3 games',lol:'Builds & runes',siege:'Ranked strategies',finals:'Loadouts & teams',wardogs:'Artillery & maps',sotf:'Island map',bo7:'Public & ranked',warzone:'Battle royale & ranked'};return `<button class="game-card theme-${e(g.theme)}" data-route="${e(g.id)}" style="--card-accent:${accent(g)}" aria-label="${e(g.name)} — ${g.status==='active'?'Open game':e(releaseStatus(g))}"><span class="game-card-art">${discoveryArtwork(g)}<span class="game-card-shade"></span>${g.status!=='active'?`<span class="game-status">${e(releaseStatus(g))}</span>`:''}</span><span class="game-card-copy"><strong>${e(g.name)}</strong><span class="game-card-cta"><span>${e(tools[g.id]||g.description)}</span>${icon(g.status==='active'?'arrow':'clock')}</span></span></button>`;}
...
async function route(...args){return transitionPage(()=>renderRoute(...args));}
async function renderRoute(id,watchGame=null,initialView=null){
...
function weaponImage(b,cls=''){const replacement=weaponArtReplacements[b.asset];if(replacement)b={...b,...replacement};return b.image?`<span class="weapon-art ${cls?'weapon-art-detail':''}"><span class="weapon-art-placeholder">Loading artwork…</span><img data-weapon-art class="${cls}" src="assets/weapons/${cls?'':'thumbs/'}${e(b.asset)}.webp" ${cls?'':`data-local-full="assets/weapons/${e(b.asset)}.webp"`} decoding="async" data-remote="${e(b.image)}" data-fallback="${e(weaponImages[(b.weaponGame||'')+'|'+b.weapon]||'')}" alt="${e(b.weapon)}" ${replacement?'title="Weapon reference image. See the attachment list for this build."':''} loading="eager"></span>`:`<div class="weapon-placeholder ${cls}">${icon('target')}<span>Artwork unavailable</span></div>`;}
function results(){
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(b.dataset.gamePage){const id=$('#game-tabs').dataset.game;return await route(b.dataset.gamePage==='videos'?'watch':['patches','operators','progression','server-status','market','damage','planner','squad'].includes(b.dataset.gamePage)?b.dataset.gamePage:id,id);}
    ...
    if(b.dataset.codBuild){
      ...
      document.querySelectorAll('.weapon-row[data-cod-build]').forEach(row=>{const item=byId.get(row.dataset.codBuild);const active=item?.weapon===selected.weapon&&item?.playstyle===selected.playstyle;row.classList.toggle('selected',active);row.setAttribute('aria-pressed',String(active));});
