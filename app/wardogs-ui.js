...
import {validPoint,clampPoint,firingSolution,impactCorrection,bearingText,parseCoordinates,coordinateText,validateBackup} from './wardogs-model.js';
...
<section class="wd-impact-controls"><h3>Adjust from impact</h3><div class="hub-action-row"><button class="hub-button secondary" data-wd-tool="impact">Mark impact</button><button class="source-link" data-wd-action="reset-impact">Reset correction</button></div><div id="wd-impact-result" role="status"></div><details><summary>Enter impact coordinates</summary>${coordinateForm('impact','Impact position')}</details></section><section class="wd-more-controls"><div class="wd-panel-body"><section>${coordinateForm('origin','Gun position')}${coordinateForm('target','Target position')}</section>
</div>
...
function updateResult(){
  ...
  document.querySelectorAll('[data-sight-display]').forEach(el=>el.innerHTML=gunnerSight(r,selectedArc(),changed));
  const correction=currentAdjustment();
  const impactOutput=$('#wd-impact-result');
  if(impactOutput)impactOutput.innerHTML=correction?`<p><strong>${Math.abs(correction.distanceDelta).toFixed(1)} m ${correction.distanceDelta>=0?'farther':'closer'} · ${Math.abs(correction.bearingDelta).toFixed(1)}° ${correction.bearingDelta>=0?'right':'left'}</strong></p><p>Last miss: ${correction.miss.toFixed(1)} m</p>`:'';
  const reset=$('[data-wd-action="reset-impact"]');if(reset)reset.disabled=!correction;
