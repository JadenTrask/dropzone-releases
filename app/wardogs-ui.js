...
function setTool(value){
  ...
  const mark=$('.wd-impact-mark');if(mark){mark.querySelector('span').textContent=tool==='impact'?'Cancel':'Mark impact';mark.setAttribute('aria-label',tool==='impact'?'Cancel marking impact':'Mark impact on the map');}
...
<section class="wd-impact-controls"><div class="wd-impact-heading"><h3>Impact correction</h3><button class="wd-impact-mark" data-wd-tool="impact" aria-label="Mark impact on the map">${mapIcon('target')}<span>Mark impact</span></button></div><div id="wd-impact-result" role="status"></div><div class="wd-impact-secondary"><details><summary>Enter coordinates</summary>${coordinateForm('impact','Impact position')}</details><button class="wd-impact-reset" data-wd-action="reset-impact" aria-label="Reset impact correction" title="Reset correction" hidden>${mapIcon('undo')}<span>Reset</span></button></div></section><section class="wd-more-controls"><div class="wd-panel-body"><section>${coordinateForm('origin','Gun position')}${coordinateForm('target','Target position')}</section>
</div>
...
function updateResult(){
  ...
  const mark=$('.wd-impact-mark');if(mark){mark.disabled=!r||r.range!=='in'||changed;mark.setAttribute('aria-pressed',String(tool==='impact'));mark.setAttribute('aria-label',tool==='impact'?'Cancel marking impact':'Mark impact on the map');}
...
document.addEventListener('click',event=>{
  ...
  const run=async()=>{
    ...
    if(!data)return;
    if(b.dataset.wdTool)setTool(b.dataset.wdTool==='impact'&&tool==='impact'?'target':b.dataset.wdTool);
