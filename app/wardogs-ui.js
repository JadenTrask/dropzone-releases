...
const position=()=>memory.positions[memory.map]||(memory.positions[memory.map]={origin:null,target:null,weapon:'mortar',arc:'single',lockOrigin:false});
...
function readSaved(){
  ...
  for(const map of maps){
    ...
    memory.positions[map.id]={origin:validPoint(p.origin,map)?p.origin:null,target:validPoint(p.target,map)?p.target:null,weapon:['mortar','spg'].includes(p.weapon)?p.weapon:'mortar',arc:['single','low','high'].includes(p.arc)?p.arc:'single',lockOrigin:p.lockOrigin===true};
  }
...
export function leaveWardogs(){active=false;++request;view?.destroy();view=null;}
...
function setTool(value){if(value==='origin'&&position().lockOrigin){toast('Unlock the gun to move it on the map.');return;}tool=value;document.querySelectorAll('[data-wd-tool]').forEach(b=>{b.classList.toggle('active',b.dataset.wdTool===tool);b.setAttribute('aria-pressed',b.dataset.wdTool===tool);});const hint=$('#wd-tool-hint');if(hint)hint.textContent={origin:'Click the map to place your gun.',target:'Click the map to place your target.',pan:'Drag to pan. Scroll to zoom. Drag a pin to move it.',ruler:'Click two points to measure distance and bearing.'}[tool];view?.draw();}
function setPoint(key,value,commit=true){
  position()[key]=clampPoint({x:Math.round(value.x*100)/100,y:Math.round(value.y*100)/100},currentMap());
  if(commit){save();if(tool===key)setTool(key==='origin'?'target':'pan');updateTargets();}
  updateResult();view?.draw();
}
...
function render(){
  if(!active)return;view?.destroy();view=null;
  ...
  $('#hub-app').innerHTML=`<main class="hub-main wardogs-page wd-fixed"><div class="wd-workspace"><aside class="wd-controls" aria-label="Firing controls"><div class="wd-sidebar-heading"><h1>Artillery calculator</h1></div><div class="wd-selectors"><label>Map<select id="wd-map" aria-label="WARDOGS map">${opts(data.maps.maps.map(m=>[m.id,m.name]),map.id)}</select></label><label>Weapon<select id="wd-weapon" aria-label="WARDOGS weapon">${opts(data.weapons.map(w=>[w.id,w.name]),p.weapon)}</select></label></div><div id="wd-result" class="wd-result" aria-live="polite" aria-atomic="true"></div>
...
<div class="wd-sidebar-footer"><button class="hub-button secondary" data-wd-action="targets">Saved targets</button><button class="source-link" data-wd-action="help">Data & help</button></div></aside><section class="wd-map-card" aria-label="Interactive ${e(map.name)} map"><div class="wd-map-toolbar"><div class="wd-tools" role="group" aria-label="Map tool">${[['origin','Gun'],['target','Target'],['pan','Pan'],['ruler','Ruler']].map(([id,label])=>`<button data-wd-tool="${id}" class="${tool===id?'active':''}" aria-pressed="${tool===id}">${label}</button>`).join('')}</div><div class="wd-view-tools"><button data-wd-action="zoom-out" aria-label="Zoom map out">−</button><button data-wd-action="zoom-in" aria-label="Zoom map in">+</button><button data-wd-action="fit">Fit map</button><button data-wd-action="frame">Frame shot</button></div></div><div class="wd-map-frame"><canvas id="wd-map-canvas" tabindex="0" aria-label="${e(map.name)} tactical map. Press G for gun, T for target, R for ruler, F to fit; arrow keys pan. Use coordinate forms for precise positions."></canvas><div id="wd-cursor-label" class="wd-cursor-label" hidden aria-hidden="true"></div><div id="wd-tool-hint" class="wd-tool-hint"></div><div id="wd-range-legend" class="wd-range-legend" aria-label="Weapon range ring legend"></div></div><div class="wd-map-footer"><span id="wd-cursor">${e(map.name)} · North is up</span><span id="wd-image-status">Loading map…</span></div><div id="wd-source-status" class="wd-source-status" role="status"></div></section></div><dialog id="wd-target-dialog" class="wd-dialog" aria-label="Saved targets"><form method="dialog" class="wd-dialog-close"><button class="hub-button secondary">Close targets</button></form><section class="wd-targets"><div class="wd-target-heading"><div><span class="hub-eyebrow">SAVED ON THIS PC</span><h2>Targets</h2></div><div class="hub-action-row"><button class="hub-button secondary" data-wd-action="export">Export targets</button><button class="hub-button secondary" data-wd-action="import">Import targets</button><input type="file" id="wd-import-file" accept=".json,application/json" hidden></div></div><form id="wd-save-target" class="wd-save-form"><label>Target name<input name="name" maxlength="80" placeholder="North bridge" required></label><label class="wd-save-origin"><input type="checkbox" name="includeOrigin" checked> Include gun position</label><button class="hub-button" type="submit">Save target</button></form><div id="wd-target-list"></div></section></dialog><dialog id="wd-help-dialog" class="wd-dialog" aria-label="Calculator data and controls"><form method="dialog" class="wd-dialog-close"><button class="hub-button secondary">Close help</button></form><p class="wd-key-help">Map shortcuts: G gun · T target · H pan · R ruler · F fit · arrows pan · + / − zoom. Esc closes this window.</p><section class="wd-method"><h3>Firing data & map sources</h3><p>Distance and compass bearing use the reference map’s coordinate calibration. One coordinate unit is 100 metres; 0.01 is one metre. Coordinates increase east and north.</p><p>Elevation is the game’s HUD MIL value from community firing tables, with interpolation between samples. These are level-ground table solutions. Terrain height, obstacles, wind, dispersion, and vehicle tilt are not modeled. Keep the SPH-2 level.</p><p>The release build has not been tested. Verify a short, middle, and long shot for each weapon when the game is available. Save the gun, intended target, displayed MIL, and actual impact coordinates when reporting a miss.</p><p>Firing tables refresh on launch and every 15 minutes. The app checks for changed map calibration; affected maps stop showing elevation solutions until an app update. Map images and the bundled grid are pinned to the same source revision.</p><p>Bakurani and Ozeti include map tiles for offline use. Closer zoom can load extra detail online from that same revision. New maps or changed game behavior may require an app update.</p><div class="hub-action-row"><button class="source-link" data-hub-source="https://wardogs-artillery.com/">Calculator reference ↗</button><button class="source-link" data-hub-source="${e(map.referenceUrl)}">${e(map.name)} on MetaForge ↗</button><button class="source-link" data-hub-source="https://github.com/apollyon-sys/wardogs-calculator">Source project & credits ↗</button></div><p>Original source and calibration: Apollyon (MIT). WARDOGS artwork and trademarks belong to their respective owners. Independent companion.</p></section></dialog></main>`;
  ...
  setTool(tool);updateSource();updateResult();updateTargets();
}
function updateSource(){
  ...
  el.innerHTML=`<span>${changed?'Map calibration changed · elevation disabled':offline?'Refresh failed · saved firing tables':'Community firing tables · release accuracy untested'}</span><span>${loading?'Checking…':'Fetched '+e(dateTime(data.fetchedAt))}</span><button class="source-link" data-wd-action="refresh" ${loading?'disabled':''}>Refresh data</button>`;
}
...
function updateResult(){
  ...
  $('#wd-result').innerHTML=`<div><span>RANGE</span><strong>${r?Math.round(r.distance).toLocaleString():'—'}<small>${r?'m':''}</small></strong><p>${w.minRange.toLocaleString()}–${w.maxRange.toLocaleString()} m supported</p></div><div><span>AZIMUTH</span><strong>${bearingText(r?.azimuth)}</strong><p>Compass bearing · 0° north</p></div><div class="wd-elevation"><span>ELEVATION · ${arc==='single'?'MORTAR':arc.toUpperCase()+' ARC'}</span><strong>${milText(v)}<small>${v?'MIL':''}</small></strong><p>${e(status)}</p></div>`;
  $('#wd-result').dataset.state=v?'ready':'waiting';
  ...
  $('#wd-arc-note').textContent=changed?'The bundled map calibration no longer matches the source.':w.id==='spg'?'Level ground table. Vehicle tilt and terrain height are not corrected.':'Level ground table. Terrain height is not corrected.';
}
...
document.addEventListener('click',event=>{
  ...
  const run=async()=>{
    ...
    if(b.dataset.wdTarget||b.dataset.wdRestore){const t=memory.targets.find(t=>t.id===(b.dataset.wdTarget||b.dataset.wdRestore));if(!t)return;position().target={...t.target};if(b.dataset.wdRestore){position().origin={...t.origin};position().weapon=t.weapon;position().arc=t.arc;save();render();}else{save();updateResult();}setTool('pan');$('#wd-target-dialog').close();view.frameShot();}
...
document.addEventListener('change',async event=>{
  ...
  try{
    if(el.id==='wd-map'){memory.map=el.value;save();tool=position().origin?'target':'origin';render();}
    ...
    if(el.id==='wd-lock'){position().lockOrigin=el.checked;if(el.checked&&tool==='origin')setTool('pan');save();view.draw();}
