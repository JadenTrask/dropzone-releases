...
class UpdateService{
  ...
  check(){
    ...
    // Bound refresh bursts so opening Dropzone does not launch every feed at once.
    let next=0;
    const run=async i=>{const job=this.jobs[i];
    ...
    const worker=async()=>{while(next<this.jobs.length)await run(next++);};
    this.running=Promise.allSettled(Array.from({length:Math.min(4,this.jobs.length)},worker)).finally(()=>{this.finishedAt=new Date().toISOString();this.running=null;});
    return this.status();
  }
}
