...
const HOSTS=new Set(['www.leagueoflegends.com','www.callofduty.com','www.reachthefinals.com','store.steampowered.com','steamcommunity.com','www.ubisoft.com','playvalorant.com','www.marvelrivals.com']);
...
function createPersonalIntel(patches,request=requestText){const cache=new Map();return async input=>{
 const names=cleanNames(input?.names);if(!['lol','bo7','warzone','finals','siege','wardogs','valorant','rivals'].includes(input?.game))throw Error('Unsupported game.');
 ...
 const url=new URL(article.url),steam=['wardogs','siege','rivals'].includes(input.game);if(url.protocol!=='https:'||!(HOSTS.has(url.hostname)||(steam&&url.hostname==='steamstore-a.akamaihd.net'))||url.username||url.password)throw Error('Unexpected publisher link.');
