...
function createPersonalIntel(patches,request=requestText){const cache=new Map();return async input=>{
 ...
 const url=new URL(article.url);if(url.protocol!=='https:'||!HOSTS.has(url.hostname)||url.username||url.password)throw Error('Unexpected publisher link.');
 let content=cache.get(article.url);if(!content||Date.now()-content.time>15*60_000){content={body:await request(article.url),time:Date.now()};cache.set(article.url,content);if(cache.size>12)cache.delete(cache.keys().next().value);}
