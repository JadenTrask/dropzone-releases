...
function intro({native=false,reduced=false}={}){
 ...
 const context={URLSearchParams,location:{search:'?intro=1'},matchMedia:()=>({matches:reduced}),document:{createElement:()=>overlay,body:{append(){},classList:{contains:()=>false}}},window:{rift:native?{desktop:true,onStartupReveal:fn=>startNative=fn}:null,addEventListener:(name,fn)=>events[name]=fn},setTimeout:(fn,ms)=>timers.push({fn,ms})};
...
test('intro cannot reveal the temporary library before the destination is ready',()=>{
 const s=intro();s.fire(750);assert.ok(!s.classes.has('dz-intro-leave'));s.ready();assert.ok(s.classes.has('dz-intro-leave'));s.fire(650);assert.equal(s.removed(),true);
});
test('ready destination remains behind the ident until animation finishes',()=>{
 const s=intro();s.ready();assert.ok(!s.classes.has('dz-intro-leave'));s.fire(750);assert.ok(s.classes.has('dz-intro-leave'));
});
test('native animation starts on the visible window event, not during hidden loading',()=>{
 const s=intro({native:true});assert.ok(!s.classes.has('dz-intro-playing'));s.ready();s.start();assert.ok(s.classes.has('dz-intro-playing'));assert.equal(s.src(),undefined);s.fire(750);assert.ok(s.classes.has('dz-intro-leave'));
});
